...
function handleSegmentClick(topic: GapTopic, modalRoot: HTMLElement) {
    ...
    // Hard-code the details for other sections to ensure consistency, as they have complex layouts.
    if (topic.title.includes('Current Laws')) {
        const lawsContent = [
    ...
    if (topic.title.includes('Outer Space Treaty')) {
        const treatyData = [
            { text: 'The exploration and use of outer space shall benefit all of mankind', image: imageAssets.img1 },
            { text: 'Outer space shall be free for exploration and use by all States', image: imageAssets.img2 },
            { text: 'Outer space is not subject to national appropriation by claim of sovereignty', image: imageAssets.img3 },
            { text: 'States shall not place nuclear weapons or weapons of mass destruction in outer space', image: imageAssets.img4 },
            { text: 'All celestial bodies shall be used exclusively for peaceful purposes', image: imageAssets.img5 },
            { text: 'Astronauts shall be regarded as the envoys of mankind', image: imageAssets.img6 },
            { text: 'States shall be responsible for governmental or non-governmental national space activities', image: imageAssets.img7 },
            { text: 'States shall be liable for damage caused by their space objects', image: imageAssets.img8 },
            { text: 'States shall avoid harmful contamination of space and celestial bodies', image: imageAssets.img9 },
        ];
        detailsContent = `<div class="space-y-4">${treatyData.map((item, index) => `
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 py-4 border-b border-cyan-500/10 last:border-b-0">
                <p class="flex-1 text-base text-gray-300 sm:text-lg leading-relaxed">${item.text}</p>
                <div class="flex flex-shrink-0 w-full sm:w-28 h-20 bg-white/5 border border-dashed border-cyan-500/30 rounded-md transition-colors hover:border-cyan-500/60 overflow-hidden">
                    <img src="${item.image}" alt="Illustration for Outer Space Treaty principle ${index + 1}" class="w-full h-full object-cover" />
                </div>
            </div>`).join('')}</div>`;
    } else if (topic.title.includes('Current Laws')) {
        const agencyLogoMap: { [key: string]: string } = {
            'NASA': imageAssets.img10,
            'ESA': imageAssets.img11,
            'DLR': imageAssets.img12
        };
        const sections = topic.details.split('\n\n');
        const frameworks = sections.slice(0, 3).map((framework) => {
            const [title, ...descriptionParts] = framework.split('\n');
            let logoSrc = '';
            if (title.includes('NASA')) logoSrc = agencyLogoMap['NASA'];
            else if (title.includes('ESA')) logoSrc = agencyLogoMap['ESA'];
            else if (title.includes('DLR')) logoSrc = agencyLogoMap['DLR'];
            
            const logoContent = logoSrc ? `<img src="${logoSrc}" alt="${title.split(':')[0].replace(/\*\*/g, '')}" class="w-full h-full object-contain p-2" />` : '';
        ...
        const useCaseImages = [imageAssets.img11, imageAssets.img12, imageAssets.img13, imageAssets.img13];
        detailsContent = `<div class="grid grid-cols-1 gap-4 sm:grid-cols-2">${topic.details.split('\n\n').map((useCase, index) => {
            ...
            return `
              <div class="p-5 bg-[#0c0a1d] border border-cyan-500/30 rounded-xl shadow-lg hover:shadow-cyan-500/20 transition-shadow duration-300 flex flex-col">
                ...
                <div class="w-full h-24 bg-white/5 border border-dashed border-cyan-500/30 rounded-md overflow-hidden flex items-center justify-center">
                    <img src="${useCaseImages[index]}" alt="${title.split(':')[0].replace(/\*\*/g, '')}" class="w-auto h-full object-contain p-2" />
                </div>
              </div>`;
        }).join('')}</div>`;
    } else {
